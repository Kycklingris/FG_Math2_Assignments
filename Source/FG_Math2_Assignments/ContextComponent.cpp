...
void UContextComponent::UpdateContext() {
	...
	if (LocalSpaceLocation.X < 0) {
	    RelatedContext.Context |= 1
				      << static_cast<uint8>(EContext::Infront);
	} else {
	    RelatedContext.Context |= 1 << static_cast<uint8>(EContext::Behind);
	}

	if (LocalSpaceLocation.Y < 0) {
	    RelatedContext.Context |= 1
				      << static_cast<uint8>(EContext::ToRight);
	} else {
	    RelatedContext.Context |= 1 << static_cast<uint8>(EContext::ToLeft);
	}

	if (LocalSpaceLocation.Z < 0) {
	    RelatedContext.Context |= 1 << static_cast<uint8>(EContext::Above);
	} else {
	    RelatedContext.Context |= 1 << static_cast<uint8>(EContext::Below);
	}
    }
}
